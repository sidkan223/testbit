...
def sensor_create(sensor, site, key, medici):
    """Get sensor by geocode. If it doesn't exist create one.

    Keyword arguments:
        sensor -- JSON sensor object
    """
    sensor_id = None
    ...
    if sensor_from_medici:
        ...
        logging.info(" [pyG.sensor_stream_create " + strftime('%Y-%m-%d %H:%M:%S') + "] POSTing sensor " + str(
            site) + " to Medici...")
...
def stream_create(sensor_id, site, stream, key, medici):
    """Check if stream exists by name. If it doesn't, create new stream.

    Keyword arguments:
        sensor_id -- id of sensor to which stream belongs
        stream -- JSON stream object
    """
    stream_id = None

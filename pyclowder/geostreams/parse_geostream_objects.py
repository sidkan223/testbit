...
# FIXME Don't know why parsing datapoints posts datapoint and parser sensors and streams does not.
...
def parse_datapoints_from_list(headers, data, geocode, sensor_id, stream_id, site, clowder_key, clowder_url, owner=None,
                               source=None, procedures=None, blocked_headers=None):
    count = 0
    while len(data) > 0:
        if count % 300 == 0:
            logging.info(" [pyG.parse_geostream_objects] Posting datapoint " + str(count) + " for " + str(
                site) + " stream " + str(stream_id))
        count += 1
        datapoint = {
            'start_time': data[0][0],
            'end_time': data[0][0],
...
def parse_single_datapoint(headers, data, start_time, geocode, sensor_id, stream_id, site, clowder_key, clowder_url,
                           owner=None, source=None, procedures=None, blocked_headers=None):
    ...
    for i in range(len(data)):
        if (blocked_headers is None or headers[i] not in blocked_headers) and data[i] not in [None, ""]:
...
def gltg_create_sensor_json(site, pretty_name, geocode, source_id, source_title, network=None):
    # FIXME can this be a flag or moved to a different method?
...
def gltg_create_stream_json(sensor_id, site, pretty_name, geocode, source_id, source_title, network=None, data_type=None):
    # FIXME can this be a flag or moved to a different method?
...
def usgs_parse_json_to_sensor(dataIn):
    """Given JSON from UGSS web service, create Geostreams API json document."""
    logging.info(" [parse_geostream_objects] getting huc codes ")
    huc_url = "http://gltg.ncsa.illinois.edu/api/huc?lat=" + str(
        dataIn['value']['timeSeries'][0]['sourceInfo']["geoLocation"]["geogLocation"]["latitude"]) + "&lng=" + str(
...
def usgs_parse_rdb_to_datapoints(dataIn, sensor, stream):
    ...
    while len(field_data) > 0:
        ...
        for j in range(3, len(field_data[0]), 2):
            if field_data[0][j] not in [None, "", " ", "eqp", "ice", "***", "ICE", "Eqp", "Fld", "Ssn", "Dis", "Ice",
                                        "Bkw"]:
